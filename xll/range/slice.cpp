...
extern "C" LPOPER __declspec(dllexport) WINAPI xll_range_slice(const LPOPER pr, LONG start, LONG stride, LONG size)
{
	...
	try {
		...
		if (po)
			*po = range::slice(*po, start, stride, size);
		else
			o = range::slice(*pr, start, stride, size);
	}
...
extern "C" LPOPER12 __declspec(dllexport) WINAPI xll_range_slice12(const LPOPER12 pr, LONG start, LONG stride, LONG size)
{
	static OPER12 o;
