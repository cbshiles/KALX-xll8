...
namespace xll {
	...
	public:
		...
		locus()
		{
			OPERX xCaller = Excel<XLOPERX>(xlfCaller);
			OPERX xReftext = Excel<XLOPERX>(xlfReftext, xCaller);

			ref_ = Excel<XLOPERX>(xlfGetDef, xReftext);
			...
			OPERX val = Excel<XLOPERX>(xlfGetName, val_);
			OPERX def = Excel<XLOPERX>(xlfGetDef, val);
		...
		OPERX get()
		{
			return Excel<XLOPERX>(xlfEvaluate, Excel<XLOPERX>(xlfGetName, val_));
		}
	...
	private:
		...
		{
			Excel<XLOPERX>(xlfSetName, name, value);
			names()[name] = value;
		}
		void Name(const OPERX& name)
		{
			Excel<XLOPERX>(xlfSetName, name);
			names().erase(name);
		}
	};
